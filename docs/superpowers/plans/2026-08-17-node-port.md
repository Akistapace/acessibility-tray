...
- Produces: `class BackendServer extends EventEmitter { config: AppConfig; previewEnabled: boolean; constructor(deps); start(); stop(); send(command): Promise<void>; onTrackingFrame(frame): Promise<void> }`. **This is the drop-in replacement for `BackendProcess`**: it emits `"message"` events shaped exactly like the old protocol messages, and exposes `.send(command)` — the same two members `ipcRelay.ts`'s `wireBackendRelay` already expects, so that file needs zero changes (Task 12 only rewires what `index.ts` constructs).
...
Run: `npx vitest run`
Expected: PASS for every remaining suite (`config`, `gestures`, `mouseController`, `clickLog`, `pointTracker`, `faceMetrics`, `trackingEngine`, `backendServer`, `ipcRelay`, `position`, `pulse`, `toggleState`, `trayState`). `npx tsc --noEmit -p tsconfig.json` will report the missing `./windows/trackingWindow` module — expected until Task 13.
