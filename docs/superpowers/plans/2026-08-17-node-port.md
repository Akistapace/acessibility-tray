...
**Files:**
- Modify: `src/modules/mouseController.ts` (append to the file created in Task 4)
- Modify: `tests/mouseController.test.ts` (append)
- Modify: `package.json` (add `@nut-tree-fork/nut-js`)
...
- [ ] **Step 2: Write failing tests (append to `tests/mouseController.test.ts`)**
...
- [ ] **Step 4: Append the driver interface and `MouseController` class to `src/modules/mouseController.ts`**
...
```bash
git add src/modules/mouseController.ts tests/mouseController.test.ts package.json package-lock.json
git commit -m "feat(modules): port mouse_controller.py's MouseController to nut-js"
```

---

### Task 10: Port `engine.py`'s `Engine` → `src/modules/trackingEngine.ts`

**Files:**
- Create: `src/modules/trackingEngine.ts`
- Test: `tests/trackingEngine.test.ts`
...
```ts
// tests/trackingEngine.test.ts
import { describe, expect, it } from "vitest";
import { TrackingEngine } from "../src/modules/trackingEngine";
import { defaultConfig } from "../src/modules/config";
import type { MouseDriver } from "../src/modules/mouseController";
import type { FaceMetrics, TrackingFrame } from "../src/modules/types";
...
Run: `npx vitest run tests/trackingEngine.test.ts`
Expected: FAIL — module not found.

- [ ] **Step 3: Implement `src/modules/trackingEngine.ts`**

```ts
import { GestureEngine } from "./gestures";
import { MouseController, type MouseDriver } from "./mouseController";
import type { AppConfig } from "./config";
import type { FaceMetrics, TrackingFrame } from "./types";
...
```bash
git add src/modules/trackingEngine.ts tests/trackingEngine.test.ts
git commit -m "feat(modules): port engine.py's Engine to TrackingEngine"
```

---

### Task 11: Port `backend.py`'s `BackendServer` → `src/modules/backendServer.ts`

**Files:**
- Create: `src/modules/backendServer.ts`
- Test: `tests/backendServer.test.ts`
...
```ts
// tests/backendServer.test.ts
import { describe, expect, it, beforeEach, afterEach } from "vitest";
import fs from "node:fs";
import os from "node:os";
import path from "node:path";
import { BackendServer } from "../src/modules/backendServer";
import { TrackingEngine } from "../src/modules/trackingEngine";
import * as configMod from "../src/modules/config";
import type { MouseDriver } from "../src/modules/mouseController";
...
Run: `npx vitest run tests/backendServer.test.ts`
Expected: FAIL — module not found.

- [ ] **Step 3: Implement `src/modules/backendServer.ts`**

```ts
import { EventEmitter } from "node:events";
import * as clickLog from "./clickLog";
import { configFromDict, configToDict, loadConfig, saveConfig, type AppConfig } from "./config";
import { triggerProgress } from "./gestures";
...
```bash
git add src/modules/backendServer.ts tests/backendServer.test.ts
...
**Files:**
- Modify: `src/modules/index.ts` (construct `BackendServer` + `TrackingEngine` + `NutJsMouseDriver` instead of `BackendProcess`/`resolveBackendCommand`)
- Delete: `src/modules/backendProcess.ts`, `src/modules/backendCommand.ts`, `src/modules/protocol.ts`
- Delete: `tests/backendProcess.test.ts`, `tests/backendCommand.test.ts`, `tests/protocol.test.ts`, `tests/fixtures/echoBackend.mjs`
- No changes needed: `src/modules/ipcRelay.ts`, `src/modules/tray.ts`, `src/modules/windows/configWindow.ts`, `src/modules/windows/buttonsWindow.ts` — all already talk to whatever object `index.ts` hands them via the `{send(message), on(event, cb)}` shape, which `BackendServer` satisfies identically to the old `BackendProcess`.
...
```bash
git rm src/modules/backendProcess.ts src/modules/backendCommand.ts src/modules/protocol.ts
git rm tests/backendProcess.test.ts tests/backendCommand.test.ts tests/protocol.test.ts
git rm -r tests/fixtures
```

- [ ] **Step 2: Rewrite `src/modules/index.ts`**
...
import { NutJsMouseDriver } from "./mouseController";
import { loadConfig } from "./config";
import { toggleTouchKeyboard } from "./win32";
...
**Files:**
- Create: `src/modules/windows/trackingWindow.ts`
- Create: `src/ui/tracking/index.html`, `src/ui/tracking/index.ts`
- Modify: `src/ui/preload/index.ts` (add the `tracking` bridge)
...
In `src/ui/preload/index.ts`, alongside the existing `backend` bridge:
...
- [ ] **Step 2: Create `src/modules/windows/trackingWindow.ts`**
...
export function createTrackingWindow(backend: BackendServer): BrowserWindow {
  ...
  win.loadFile(path.join(__dirname, "..", "..", "ui", "tracking", "index.html"));
...
In `src/modules/backendServer.ts`, add a listener list and fire it whenever `set_preview` changes the flag:
...
- [ ] **Step 4: Create `src/ui/tracking/index.html`**
...
- [ ] **Step 5: Create `src/ui/tracking/index.ts`**
...
```bash
git add src/modules/windows/trackingWindow.ts src/modules/backendServer.ts src/ui/tracking src/ui/preload/index.ts
...
**Files:**
- Modify: `src/ui/tracking/index.ts`
- Create: `src/ui/tracking/assets/` (vendored model + WASM assets)
...
mkdir -p src/ui/tracking/assets
curl -L -o src/ui/tracking/assets/face_landmarker.task \
  https://storage.googleapis.com/mediapipe-models/face_landmarker/face_landmarker/float16/latest/face_landmarker.task
cp -r node_modules/@mediapipe/tasks-vision/wasm src/ui/tracking/assets/wasm
```

Both are static binary assets loaded from a local path at runtime (never fetched from Google's CDN), so the app stays offline-capable exactly like the PyInstaller-bundled MediaPipe data it replaces. `scripts/copyStaticAssets.mjs` already copies everything under `src/ui` except `.ts` files (Task 1), so these ship automatically.

- [ ] **Step 2: Replace the stub in `src/ui/tracking/index.ts`**
...
```bash
git add src/ui/tracking/index.ts src/ui/tracking/assets package.json package-lock.json .gitignore
git commit -m "feat(ui): wire MediaPipe FaceLandmarker into the tracking loop"
```

Note: `src/ui/tracking/assets/wasm` and `face_landmarker.task` are binary vendored assets — if the repo's `.gitignore` excludes `dist/` broadly, double-check it doesn't also exclude `src/ui/tracking/assets/` before committing (it shouldn't, since that pattern is dist-only, but confirm rather than assume).
...
**Files:**
- Create: `src/ui/tracking/pointTracker.ts`
- Modify: `src/ui/tracking/index.ts`
- Modify: `package.json` (add `@techstark/opencv-js`)

**Interfaces:**
- Consumes: `prunePoints`, `shouldAddPoint`, `meanMovement`, `Point` (Task 6, `src/modules/pointTracker.ts`).
...
- [ ] **Step 2: Implement `src/ui/tracking/pointTracker.ts`**

```ts
import cv from "@techstark/opencv-js";
import { meanMovement, prunePoints, shouldAddPoint, type Point } from "../../modules/pointTracker";
...
- [ ] **Step 3: Wire it into `src/ui/tracking/index.ts`**
...
```bash
git add src/ui/tracking/pointTracker.ts src/ui/tracking/index.ts package.json package-lock.json
...
- [ ] **Step 1: Add a preview canvas and overlay drawing to `src/ui/tracking/index.ts`**
...
```bash
git add src/ui/tracking/index.ts
...
**Files:**
- Create: `src/modules/keyboard.ts`
- Modify: `src/modules/index.ts` (replace the `onAction` stub, pass `toggleVoiceTyping` to `BackendServer`)
...
- [ ] **Step 1: Implement `src/modules/keyboard.ts`**
...
- [ ] **Step 2: Wire the action-feedback loop and voice typing into `src/modules/index.ts`**

Add imports:

```ts
import * as clickLog from "./clickLog";
import { toggleVoiceTyping } from "./keyboard";
```
...
```bash
git add src/modules/keyboard.ts src/modules/index.ts
...
- [ ] **Step 1: Rewrite `electron-builder.yml`**
...
This drops the `extraResources` block entirely (no more `facemesh-mouse-backend.exe` to stage) and fixes `icon`'s path from `../assets/icon.ico` to `assets/icon.ico` now that `electron-builder.yml` and `assets/` are both repo-root siblings (Task 1's flatten).
...
Expected: builds cleanly, produces `release/FaceMesh Mouse Setup <version>.exe` with no PyInstaller step and no backend-exe staging. Install it and run it once to confirm the packaged app launches (camera prompt, tray icon, config window on first run).
...
**Files:**
- Delete (git-tracked): `run.py`, `pyproject.toml`, `requirements.txt`, `requirements-dev.txt`, `backend.spec`, `src/facemesh_mouse/` (whole package), `tests/*.py`, `assets/generate_icon.py`
...
- Modify: `README.md`
- Leave untouched: `config.json`, `config.json.keep`, `clicks.log` — runtime/user data, unrelated to the language the engine is written in.
...
```bash
git rm run.py pyproject.toml requirements.txt requirements-dev.txt backend.spec assets/generate_icon.py
git rm -r src/facemesh_mouse tests
```

Note: `tests/` at this point contains only the `.py` files (`test_backend.py`, `test_click_log.py`, `test_config.py`, `test_gestures.py`, `test_ipc_protocol.py`, `test_mouse_controller.py`, `test_point_tracker.py`, `test_preview.py`, `test_tracker.py`, `test_virtual_keyboard.py`, `test_voice_typing.py`) — the TypeScript `tests/*.test.ts` files created in Tasks 2–11 and moved in Task 1 live in the repo-root `tests/` directory too. Run `git rm -r src/facemesh_mouse` and individually `git rm tests/test_*.py` (not `git rm -r tests`) so the `.ts` suites are not touched:
...
O instalador fica em `release/FaceMesh Mouse Setup <versão>.exe`.
Pontos de atenção:
