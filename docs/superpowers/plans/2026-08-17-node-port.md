...
**Goal:** Remove Python entirely (mediapipe/opencv/pynput/ctypes) and reimplement the whole tracking/gesture/mouse engine in TypeScript running inside Electron. Tasks 1-8 originally did this by flattening `electron/` into the repo root as `src/modules/` (main-process logic) + `src/ui/` (everything Chromium-rendered); that flat layout has since been superseded by a pnpm-workspace monorepo, and Tasks 9-19 target the current layout instead: `apps/desktop/` (this app, `src/main` + `src/renderer`) + `packages/shared` (cross-cutting types), per `docs/superpowers/specs/2026-08-18-node-port-monorepo-layout-design.md`.
...
**Spec:** `docs/superpowers/specs/2026-08-17-node-port-design.md` (builds on `docs/superpowers/specs/2026-08-15-electron-migration-design.md`). Its "Folder layout" section specifically is superseded by `docs/superpowers/specs/2026-08-18-node-port-monorepo-layout-design.md`, which defines the current `apps/desktop` + `packages/shared` monorepo shape (targeted from Task 9 onward) — every other section of the 2026-08-17 spec (Architecture, Component mapping, Native integration, Camera pipeline requirements, What gets deleted, Testing, Packaging, Out of scope) remains authoritative.
...
- Run `npm test` (all suites) and `npx tsc --noEmit -p apps/desktop/tsconfig.json` (and `-p apps/desktop/tsconfig.renderer.json` once it has content) after every task; both must be clean before commit.

---

## File Structure

Tasks 1-8 originally targeted a flat repo-root `src/modules/` + `src/ui/`
layout, described in the past tense inside those tasks' own bodies below.
That layout was superseded by a pnpm-workspace monorepo before Task 9, and
the current target for every file this plan creates or references from
Task 9 onward is `apps/desktop/src/main/...` (services get a `.service.ts`
suffix, except `backendProcess.ts`, `backendCommand.ts`, `protocol.ts`, and
`trayState.ts`, which keep their plain names), `apps/desktop/src/renderer/...`,
and `packages/shared/src/...` for the cross-cutting `FaceMetrics`/
`TrackingFrame` types. See the "Target layout" section of
`docs/superpowers/specs/2026-08-18-node-port-monorepo-layout-design.md` for
the full annotated tree — it is not duplicated here to avoid the two copies
...
> Editorial note, resolved: an earlier pass of the monorepo-layout spec's diagram showed a single merged `services/engine.service.ts`, contradicting this task's and Task 11's already-detailed, already-tested split into `trackingEngine.service.ts` + `backendServer.ts`. Ruling: keep the split. It matches the original Python structure (`engine.py`'s `Engine` and `backend.py`'s `BackendServer` were always two separate classes/files), it's better-separated (per-frame gesture/mouse math vs. IPC command dispatch are genuinely different concerns), and rewriting two fully-specified, fully-tested tasks to match a one-line diagram label would discard real design work for no benefit. The spec's diagram has been corrected to show both files instead.
...
**Files:**
- Create: `apps/desktop/src/main/services/keyboard.service.ts`
...
- [ ] **Step 1: Implement `apps/desktop/src/main/services/keyboard.service.ts`**
...
```bash
git add apps/desktop/src/main/services/keyboard.service.ts apps/desktop/src/main/index.ts
