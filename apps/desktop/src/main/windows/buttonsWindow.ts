...
export function moveButtonsWindow(dx: number, dy: number): void {
  if (!win) return;
  // Electron's native setPosition throws (crashing the whole main process,
  // uncaught) on a non-finite value -- a malformed or missing dx/dy must be
  // dropped here rather than handed straight through.
